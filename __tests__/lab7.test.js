describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let root = await prodItem.getProperty("shadowRoot");
    let toClick = await root.$("button");
    await toClick.click();
    let text = await page.evaluate(button => button.innerText, toClick);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let count = await page.evaluate(span => span.innerText, cart);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length;  i++){
      let root = await prodItems[i].getProperty("shadowRoot");
      let clicked = await root.$("button");
      let text = await page.evaluate(button => button.innerText, clicked);
      expect(text).toBe("Remove from Cart");
    }
  }, 10000);
